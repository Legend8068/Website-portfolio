...
export default function DecryptedText({
  ...
  return (
    <motion.span
      ref={containerRef}
      style={{ display: 'inline-block', whiteSpace: 'pre-wrap', ...parentStyle }}
      className={parentClassName}
      {...hoverProps}
      {...props}
    >
      <span style={{ position: 'absolute', width: '1px', height: '1px', overflow: 'hidden', clip: 'rect(0,0,0,0)', whiteSpace: 'nowrap', border: 0 }}>
        {displayText}
      </span>
      <span aria-hidden="true">
        {displayText.split('').map((char, index) => {
          ...
          return (
            <span
              key={index}
              style={isRevealedOrDone ? style : { opacity: 0.7, ...encryptedStyle }}
